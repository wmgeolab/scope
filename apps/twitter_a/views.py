...
def twitter_import(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for x in tweets:
                vals.append(TwitterSource.objects.create(source_search_id=x.source_id+datetime.datetime.now().strftime("%m/%d/%Y-%H:%M:%S"), source_id=x.source_id, source_text=x.source_text, source_date=x.source_created_at, source_url=x.source_url))
