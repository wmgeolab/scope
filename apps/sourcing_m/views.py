...
def home(request):
    ...
    return render(request, 'templates/sourcing_m/home.html', {'sources':sources})
...
def source_view(request, pk):
    ...
    return render(request, 'templates/sourcing_m/source_view.html', {'form':form, 'pk':pk})


def source_add(request):
    if request.method == 'GET':
        form = SourceForm()
        return render(request, 'templates/sourcing_m/source_add.html', {'form':form})
    elif request.method == 'POST':
        form = SourceForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('sourcing_m')
        else:
            print(form.errors)
...
def source_import(request):
    if request.method == 'GET':
        return render(request, 'templates/sourcing_m/source_import.html')
    elif request.method == 'POST':
        ...
        return render(request, 'templates/sourcing_m/source_import.html', {'importresults':importresults})
