...
from .forms import DomainForm, ActivityCodeForm, ActivitySubcodeForm, ActorCodeForm, ActorRoleForm, DateCodeForm, StatusCodeForm, FinancialCodeForm, SourceCodeForm, TriggerWordForm
...
def overview(request):
    ...
    actorroles = ActorRole.objects.all()
    datecodes = DateCode.objects.all()
    ...
    return render(request, 'templates/domain/overview.html',
    	...
    	'actorcodes':actorcodes, 'actorroles':actorroles, 'datecodes':datecodes,
    	'statuscodes':statuscodes, 'financialcodes':financialcodes, 'sourcecodes':sourcecodes,
...
def edit_actorroles(request):
	if request.method == 'GET':
		return render(request, 'templates/domain/edit_actorroles.html')
	elif request.method == 'POST':
		logger = [] # each entry will become a new line in a textstring
		uploaded_file = request.FILES['importdocument']
		import csv
		...
		# finish up
		logger.append('import finished')
		logger.append('successfully imported {} actor role codes'.format(count))
		importresults = '\n'.join(logger) # make logs into a multiline textstring
		return render(request, 'templates/domain/edit_actorroles.html', {'importresults':importresults})

def edit_datecodes(request):
	if request.method == 'GET':
		return render(request, 'templates/domain/edit_datecodes.html')
	elif request.method == 'POST':
		logger = [] # each entry will become a new line in a textstring
		uploaded_file = request.FILES['importdocument']
		import csv
		...
		# finish up
		logger.append('import finished')
		logger.append('successfully imported {} date codes'.format(count))
		importresults = '\n'.join(logger) # make logs into a multiline textstring
		return render(request, 'templates/domain/edit_datecodes.html', {'importresults':importresults})
